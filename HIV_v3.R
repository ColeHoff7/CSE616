...
HIV = function(n, probHIV, probInfect, probEmpty, probReplace, rank, t) {
  ...
  for (i in 2:(t+1)) {
    ...
    body = applyExtended(bodyExtended, probInfect, probReplace, respoProbTime[i], mode, rank, i-1)
    grids[,,i] = body
  }
...
applyExtended = function(latExt, probInfect, probReplace, respoProbTime, mode, rank, layer) {
  ...
  for (j in 2:(n + 1)) {
    for (i in 2:(n + 1)) {
      ...
      SW = newmat1[i+1, j-1]
      SE = newmat1[i+1, j+1]
      newmat2[i - 1, j - 1] = spread(site, N, NE, NW, E, S, W, SW, SE, probInfect, probReplace, i, j, mode, respoProbTime, rank)
    }
  }
  return(newmat2)
}
...
showGraphs = function(graphList, n) {
  ...
  for (k in 1:m) {
    ...
    points(empty[[1]],empty[[2]],col="white",pch=20,bg="white", cex = pointSize)
...
## test grids = HIV(n, probHIV, probInfect, probEmpty, probReplace, rank, t)
time = 156
num = 200
grids = HIV(num, .05, 0.00001, .2, 0.99, 8, time)
